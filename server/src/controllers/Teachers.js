...
async function registerTeacher(req,res){
    const {email, password, name, age, socialProfileLink, phoneNumber, profilePicture, aboutMe, canTeach} = req.body
    bcrypt.hash(password, SALT_ROUNDS, async function(err, hashedPassword) {
        const registerTeacherResponse = await TeachersService.registerTeacher(
            email, hashedPassword, name, age, socialProfileLink, phoneNumber, profilePicture, aboutMe, canTeach)
